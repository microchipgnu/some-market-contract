...
class Offer {
  constructor(token_id, nft_contract_id, amount, account_id, affiliate_id) {
...
let Market = (_dec = NearBindgen({
  requireInit: true
}), _dec2 = initialize(), _dec3 = view(), _dec4 = call({}), _dec5 = call({}), _dec6 = call({
  payableFunction: true
}), _dec7 = call({
  privateFunction: true
}), _dec(_class = (_class2 = class Market {
  constructor() {
    this.offers = new LookupMap("o");
    this.approval_ids = new LookupMap("a");
  ...
  make_offer({
    ...
    const offer = new Offer(token_id, nft_contract_id, amount, accountId, affiliate_id);
  ...
  buy({
    ...
    const deposit = attachedDeposit().valueOf();
    const price = BigInt(amount);
    assert(price === deposit, `Offer amount differs from what was actually deposited.`);
    assert(account_id === creator_account_id, `Sender ID differs from Offerer ID.`);
...
function internal_resolve_purchase() {
  ...
  const _contract = Market._create();
  if (_state) {
    Market._reconstruct(_contract, _state);
  }
  const _args = Market._getArgs();
  const _result = _contract.internal_resolve_purchase(_args);
...
function buy() {
  ...
  const _args = Market._getArgs();
  const _result = _contract.buy(_args);
  Market._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Market._serialize(_result, true));
}
